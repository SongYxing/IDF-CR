...
class TrainDataset(data.Dataset):
    ...
    def __init__(self, config):
        ...
        # self.imlist = random.sample(self.imlist,len(self.imlist) // 80)
    ...
    def __getitem__(self, index):
        ...
        sar = cv2.imread(os.path.join(self.config.datasets_dir, 'mask', str(self.imlist[index])), 0).astype(
            np.float32)
        sar = np.reshape(sar, (256, 256, 1))
...
class TestDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        sar = cv2.imread(os.path.join(self.test_dir, 'mask', filename), 0).astype(np.float32)
        sar = np.reshape(sar, (256, 256, 1))
