...
class SPANet_RSTB(nn.Module):
    def __init__(self):
        ...
        self.patch_embed = PatchEmbed(
            img_size=384, patch_size=1, in_chans=96, embed_dim=96,
        ...
        self.conv_first = nn.Conv2d(3, 96, 3, 1, 1)
...
class SPANet_RSTB_woCA(nn.Module):
    def __init__(self):
        img_size = 64,
        patch_size = 1,
        embed_dim = 96,
        ...
        use_checkpoint = False,
        resi_connection = '1conv',
        super(SPANet_RSTB_woCA, self).__init__()
        self.patch_norm = True
        self.patch_embed = PatchEmbed(
...
#
#         Attention4 = self.SAM(x)
#         x = F.relu(self.res_block13(x) * Attention4 + x)
#         x = F.relu(self.res_block14(x) * Attention4 + x)
#         x = F.relu(self.res_block15(x) * Attention4 + x)
#
#         x = F.relu(self.res_block16(x) + x)
#         x = F.relu(self.res_block17(x) + x)
#         x = self.conv_out(x)
#         x = x + self.mean
#         return Attention4, x
