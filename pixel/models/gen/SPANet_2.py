...
class SPANet_RSTB(nn.Module):
    def __init__(self):
        ...
        self.patch_embed = PatchEmbed(
            img_size=512, patch_size=1, in_chans=96, embed_dim=96,
        ...
        self.SAM1 = SAM(96, 96, 1)
        self.conv_before_upsample = nn.Sequential(
            nn.Conv2d(96, 96, 3, 1, 1),
            nn.LeakyReLU(inplace=True)
        )
        self.conv_first = nn.Conv2d(4, 96, 3, 1, 1)

        self.conv_last = nn.Conv2d(96, 3, 3, 1, 1)
        ...
        self.SAM = SAM(96, 96, 1)
        self.conv_after_body = nn.Conv2d(96, 96, 3, 1, 1)
    ...
    def forward(self, x, mask):
        ...
        x = self.conv_first(x)
        Attention, x = self.forward_features(x)
        x = self.conv_after_body(x) * Attention + x
    ...
    #     Attention, out = self.forward_features(x_first)
...
#         self.patch_norm = True
#         self.patch_embed = PatchEmbed(
#             img_size=64, patch_size=1, in_chans=embed_dim, embed_dim=embed_dim,
#             norm_layer=norm_layer if self.patch_norm else None
#         )
#         self.patch_unembed = PatchUnEmbed(
#             img_size=64, patch_size=1, in_chans=embed_dim, embed_dim=embed_dim,
...
#
#         self.SAM1 = SAM(embed_dim, embed_dim, 1)
#         self.SAM2 = SAM(embed_dim, embed_dim, 1)
#         self.SAM3 = SAM(embed_dim, embed_dim, 1)
#         self.SAM4 = SAM(embed_dim, embed_dim, 1)
#
#         self.res_block1 = Bottleneck(embed_dim, embed_dim)
#         self.res_block2 = Bottleneck(embed_dim, embed_dim)
#         self.res_block3 = Bottleneck(embed_dim, embed_dim)
#         self.res_block4 = Bottleneck(embed_dim, embed_dim)
#         self.res_block5 = Bottleneck(embed_dim, embed_dim)
#         self.res_block6 = Bottleneck(embed_dim, embed_dim)
#         self.conv_in = nn.Sequential(
#             conv3x3(3, embed_dim),
#             nn.ReLU(True)
#         )
#
#         self.conv_last_3 = nn.Conv2d(2 * embed_dim, embed_dim, 3, 1, 1)
#         self.conv_last_2 = nn.Conv2d(embed_dim, embed_dim, 3, 1, 1)
#         self.conv_last_1 = nn.Conv2d(embed_dim, 3, 3, 1, 1)
#
#     def forward_features(self, x: torch.Tensor) -> Tuple[Any, Any]:  # local
#         x_size = (x.shape[2], x.shape[3])
#         x = self.patch_embed(x)
#         x = self.pos_drop(x)
#         for layer in self.layers:
...
#         return x
#
#     # def forward_features(self, x: torch.Tensor) -> Tuple[Any, Any]: #local2
#     #     x_size = (x.shape[2], x.shape[3])
#     #     for layer in self.layers:
#     #         x = self.patch_embed(x)
#     #         x = self.pos_drop(x)
...
#     def forward(self, x, sar):
#
#         sar = self.conv_first(sar)
#         sar = self.conv_after_body(self.forward_features(sar))
#         sar = self.conv_before_upsample(sar)
#         sar = self.lrelu(self.conv_up1(torch.nn.functional.interpolate(sar, scale_factor=2, mode='nearest')))
#         sar = self.lrelu(self.conv_up2(torch.nn.functional.interpolate(sar, scale_factor=2, mode='nearest')))
#         sar = self.lrelu(self.conv_hr(sar))
#
#         out = self.conv_in(x)
#         out = self.lrelu(self.res_block1(out) + out)
#         Attention1 = self.SAM1(out)
#         out = self.lrelu(self.res_block2(out) * Attention1 + out)
#
#         Attention2 = self.SAM2(out)
#         out = self.lrelu(self.res_block3(out) * Attention2 + out)
#         Attention3 = self.SAM3(out)
#         out = self.lrelu(self.res_block4(out) * Attention3 + out)
#         Attention4 = self.SAM4(out)
#         out = self.lrelu(self.res_block5(out) * Attention4 + out)
#         out = self.lrelu(self.res_block6(out) + out)
#
#         x = torch.cat((out, sar), dim=1)
#         x = self.conv_last_3(x)
...
class Generator(nn.Module):
    def __init__(self, gpu_ids):
        ...
        # self.gen = nn.Sequential(OrderedDict([('gen', SPANet())]))
        self.gen = SPANet_RSTB()
        self.gen.apply(weights_init)

    def forward(self, x, sar):
        self.gen.to(device=self.device)
        return self.gen(x, sar)
