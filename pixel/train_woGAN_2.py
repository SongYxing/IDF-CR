...
from data_manager_2 import TrainDataset
...
def train(config):
    ...
    for epoch in range(1, config.epoch + 1):
        ...
        for iteration, batch in enumerate(training_data_loader, 1):
            real_a_cpu, real_b_cpu, M_cpu, sar_cpu = batch[0], batch[1], batch[2], batch[3]
            # real_a.data.resize_(real_a_cpu.size()).copy_(real_a_cpu)
            # real_b.data.resize_(real_b_cpu.size()).copy_(real_b_cpu)
            # real_sar.data.resize_(sar_cpu.size()).copy_(sar_cpu)
            ...
            with torch.no_grad():
                ...
                real_sar.resize_(sar_cpu.size()).copy_(sar_cpu)
                M.resize_(M_cpu.size()).copy_(M_cpu)

            att, fake_b = gen.forward(real_a, real_sar)
            # fake_b = gen.forward(real_a, real_sar)
            ################
            ### Update D ###
            ################
...
if __name__ == '__main__':
    with open('config_2.yml', 'r', encoding='UTF-8') as f:
